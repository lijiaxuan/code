...
* @Last Modified time: 2016-03-09 17:14:40
*/
#include "pid.h"
...
{
	unsigned int k = 0;
    f_open(&fil,"pid",FA_READ);

    if(fil.fs == 0){return;}
    f_read(&fil, (char *)pid.pid_list, sizeof(struct Pid_list), &k);
    file_close();
}
void pid_save()
{
	  unsigned int k = 0;
    f_open(&fil,"pid",FA_WRITE | FA_CREATE_ALWAYS);
    if(fil.fs == 0){return;}
    f_write(&fil, (char *)pid.pid_list, sizeof(struct Pid_list), &k);
    file_close();
}
