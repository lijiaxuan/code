...
float stop_engine_angle=80;
float stop_engine=80;

int stopCount=0;
int stopStatus=0;

static char hasstop = 0;
void stop(void)
{
    if(stopStatus && hasstop == 0)
    {
        //SET_ANGLE(90,2);  
      TIM4->CCR3 = 0x690;  
			return;
...
//              }	
        ...
        else if( (Point_NowNum>Route[22-1]+1 &&Point_NowNum<Route[24-1]+70) || (Point_NowNum>Route[22+25-1]+1 &&Point_NowNum<Route[24+25-1]+70) )//山道3直线
         {
									if(hasstop != 0)
									{return;}
                 brakesec=5;
...
//          
         ...
         if(stop_engine_angle>100)
             stop_engine_angle=100;
