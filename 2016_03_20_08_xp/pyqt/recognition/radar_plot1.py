...
import subprocess
import numpy as np
datatype = np.dtype([('x1','<f'),('y1','<i'),('x2','<f'),('y2','<i')])
try:
...
try:
    subprocess.Popen('python radar_plot2.py',shell=False)
except Exception:
    pass
...
class TestDialog(QDialog,Ui_Dialog):
    ...
    def update(self,data):
        ...
        if len(self.content) <= self.framelenth:
            return
        d = self.content.split(struct.pack('4B',0x01,0x01,0x01,0x01))
        try:
            # print len(d[1])
            # print str(d[1])
            x_,y_,angle_ = struct.unpack('<3f',d[1])
            self.m_x.setText(str(x_))
            self.m_y.setText(str(y_))
            self.m_angle.setText(str(angle_))
        except Exception:
            pass
        frm = d[0].split(self.spliter)
    ...
    def display(self,data):
        ...
        nparr = np.frombuffer(data,dtype=datatype)
        #print nparr
        self.logfile.write(str(nparr)+'\n')
        f = open('point','w')
        f.write(str(nparr)+'\n')
        f.close()
